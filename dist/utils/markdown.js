// 코드 블록을 마크다운으로 변환
export function codeBlockToMarkdown(block) {
...
// 디자인 결정을 마크다운으로 변환
export function designDecisionToMarkdown(decision) {
...
// 플랫폼별 마크다운 변환
export function normalizeMarkdownForPlatform(markdown, platform) {
    switch (platform) {
        case 'notion':
...
// Notion용 변환
function normalizeForNotion(markdown) {
...
// GitHub Wiki용 변환
function normalizeForGitHubWiki(markdown) {
    let normalized = markdown;
    // 내부 링크를 Wiki 스타일로 변환
...
// Obsidian용 변환
function normalizeForObsidian(markdown) {
    let normalized = markdown;
