...
import { getSession, listSessions, SessionData, StoredSession } from '../core/sessionStorage.js';
...
export interface ExportSessionInput {
  sessionIds?: string[];           // 특정 세션 ID들 (없으면 전체)
  format: ExportFormat;            // 출력 형식
  outputPath?: string;             // 출력 경로 (없으면 반환만)
  includeMetadata?: boolean;       // 메타데이터 포함
  includeCodeBlocks?: boolean;     // 코드 블록 포함
  includeDesignDecisions?: boolean; // 설계 결정 포함
  template?: 'default' | 'minimal' | 'detailed' | 'report';
  title?: string;                  // 문서 제목
  bundleMultiple?: boolean;        // 여러 세션을 하나로 합치기
}
...
function generateMarkdown(
  ...
  const {
    ...
    template = 'default',
    title = 'Vibe Coding Session Export',
    bundleMultiple = true
  } = options;
...
export async function exportSessionTool(input: ExportSessionInput): Promise<ExportSessionOutput> {
  ...
  try {
    ...
    exportLogger.error('Failed to export sessions', error as Error);
    return {
      success: false,
      format,
      sessionCount: 0,
